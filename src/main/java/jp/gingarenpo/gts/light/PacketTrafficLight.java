 ...
 */
public class PacketTrafficLight implements IMessage, IMessageHandler<PacketTrafficLight, IMessage> {
	
	/**
	 * TileEntityを作成する目的で保持する必要がある
	 */
	private TileEntityTrafficLight te;
	 ...
	 */
	private TrafficLight tlData;
	
	/**
	 * パケットを新規作成するためのコンストラクタ。ソースからの利用はまず行わない。
	 */
	public PacketTrafficLight() {}
	 ...
	 * @param te
	 */
	public PacketTrafficLight(TileEntityTrafficLight te) {
		this.te = te;
	 ...
	 */
	@Override
	public IMessage onMessage(PacketTrafficLight message, MessageContext ctx) {
		World world = ctx.getServerHandler().player.world; // サーバー側からプレイヤーのいるワールドを取得する
		TileEntityTrafficLight te = message.getTileEntityFromPacket(world); // パケットからTileEntityを取得する
		te.setData(tlData); // 最新の情報をセットする
	 ...
	 * @return
	 */
	public TileEntityTrafficLight getTileEntityFromPacket(World world) {
		...
		if (te == null) {
			...
			for (TileEntity load: world.loadedTileEntityList) {
				...
				if (pos.equals(load.getPos()) && load instanceof TileEntityTrafficLight) {
					// 場所が一致した場合はそれがこの場所のTileEntityである（ただしインスタンスも一致している必要がある）
					return (TileEntityTrafficLight) load;
				}
			}
		}
